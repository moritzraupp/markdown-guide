# Markdown Wiki Guide

This document provides a complete overview of **Markdown (.md) syntax and features**.
Markdown is a lightweight markup language used to format plain text.

---

## Table of Contents

1. [Document Structure](./structure.md)
   - [Headings](./structure.md#headings)
   - [Paragraphs](./structure.md#paragraphs)
   - [Line Breaks](./structure.md#line-breaks)
2. [Text Formatting](./text.md)
   - Emphasis
   - Text Formats
   - Footnotes
   - Lists
   - Tables
3. [Referencing](./referencing.md)
   - Files
   - Media
   - Links
4. [HTML](./html.md)
5. [General Guidelines](./guide.md) <span style="color: red;">TBA</span>

---

<a href="https://gist.github.com/rxaviers/7360908" target="_blank" rel="noopener noreferrer">
  😄🔗 List of github compatible markdown emoji markup 
</a> by <a href="https://gist.github.com/rxaviers" target="_blank" rel="noopener noreferrer">Rafael Xavier de Souza</a>

---

[:arrow_up: Back to Top](#table-of-contents) 